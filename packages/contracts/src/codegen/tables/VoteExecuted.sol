...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("VoteExecuted")))
);
ResourceId constant VoteExecutedTableId = _tableId;
...
library VoteExecuted {
   ...
   */
  function set(uint256 id, uint256 timestamp, uint32 gameId, string memory voter, string memory recipient) internal {
    bytes memory _staticData = encodeStatic(timestamp, gameId);

    PackedCounter _encodedLengths = encodeLengths(voter, recipient);
    bytes memory _dynamicData = encodeDynamic(voter, recipient);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(uint256 id, uint256 timestamp, uint32 gameId, string memory voter, string memory recipient) internal {
    bytes memory _staticData = encodeStatic(timestamp, gameId);
   ...
   */
  function set(uint256 id, VoteExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);

    PackedCounter _encodedLengths = encodeLengths(_table.voter, _table.recipient);
    bytes memory _dynamicData = encodeDynamic(_table.voter, _table.recipient);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, VoteExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
   ...
   */
  function set(IStore _store, uint256 id, VoteExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
  ...
  function decodeDynamic(
    ...
    bytes memory _blob
  ) internal pure returns (string memory voter, string memory recipient) {
    uint256 _start;
    uint256 _end;
    unchecked {
      _end = _encodedLengths.atIndex(0);
    }
    voter = (string(SliceLib.getSubslice(_blob, _start, _end).toBytes()));

    _start = _end;
    unchecked {
      _end += _encodedLengths.atIndex(1);
    }
    recipient = (string(SliceLib.getSubslice(_blob, _start, _end).toBytes()));
  }
  ...
  ) internal pure returns (VoteExecutedData memory _table) {
    (_table.timestamp, _table.gameId) = decodeStatic(_staticData);

    (_table.voter, _table.recipient) = decodeDynamic(_encodedLengths, _dynamicData);
  }
   ...
   */
  function encodeDynamic(string memory voter, string memory recipient) internal pure returns (bytes memory) {
    return abi.encodePacked(bytes((voter)), bytes((recipient)));
  }
