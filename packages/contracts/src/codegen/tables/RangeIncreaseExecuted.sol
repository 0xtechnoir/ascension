...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("RangeIncreaseExe")))
);
ResourceId constant RangeIncreaseExecutedTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x0024020120040000000000000000000000000000000000000000000000000000
);

struct RangeIncreaseExecutedData {
  uint256 timestamp;
  uint32 gameId;
  string player;
}

library RangeIncreaseExecuted {
  /**
   ...
   */
  function set(uint256 id, RangeIncreaseExecutedData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, RangeIncreaseExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
   ...
   */
  function set(IStore _store, uint256 id, RangeIncreaseExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
  ...
  ) internal pure returns (RangeIncreaseExecutedData memory _table) {
