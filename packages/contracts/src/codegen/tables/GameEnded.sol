...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("GameEnded")));
bytes32 constant GameEndedTableId = _tableId;
...
library GameEnded {
  ...
  function emitEphemeral(bytes32 key, uint256 timestamp) internal {
  ...
  function emitEphemeral(IStore _store, bytes32 key, uint256 timestamp) internal {
    bytes memory _data = encode(timestamp);
