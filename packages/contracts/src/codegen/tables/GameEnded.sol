...
library GameEnded {
   ...
   */
  function set(bytes32 key, uint256 timestamp) internal {
   ...
   * @notice Set the full data using individual values.
   */
  function _set(bytes32 key, uint256 timestamp) internal {
    bytes memory _staticData = encodeStatic(timestamp);
   ...
   */
  function set(IStore _store, bytes32 key, uint256 timestamp) internal {
    bytes memory _staticData = encodeStatic(timestamp);
   ...
   *
   */
  function decode(bytes memory _staticData, PackedCounter, bytes memory) internal pure returns (uint256 timestamp) {
    (timestamp) = decodeStatic(_staticData);
  }
   ...
   */
  function encode(uint256 timestamp) internal pure returns (bytes memory, PackedCounter, bytes memory) {
    bytes memory _staticData = encodeStatic(timestamp);
