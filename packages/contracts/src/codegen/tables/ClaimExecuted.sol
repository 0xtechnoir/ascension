...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("ClaimExecuted")))
);
ResourceId constant ClaimExecutedTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x0020010220000000000000000000000000000000000000000000000000000000
);

struct ClaimExecutedData {
  uint256 timestamp;
  string player;
  string gameId;
}

library ClaimExecuted {
  /**
   ...
   */
  function set(uint256 id, uint256 timestamp, string memory player, string memory gameId) internal {
    bytes memory _staticData = encodeStatic(timestamp);

    PackedCounter _encodedLengths = encodeLengths(player, gameId);
    bytes memory _dynamicData = encodeDynamic(player, gameId);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(uint256 id, uint256 timestamp, string memory player, string memory gameId) internal {
    bytes memory _staticData = encodeStatic(timestamp);
   ...
   */
  function set(IStore _store, uint256 id, uint256 timestamp, string memory player, string memory gameId) internal {
    bytes memory _staticData = encodeStatic(timestamp);
   ...
   */
  function set(uint256 id, ClaimExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp);

    PackedCounter _encodedLengths = encodeLengths(_table.player, _table.gameId);
    bytes memory _dynamicData = encodeDynamic(_table.player, _table.gameId);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, ClaimExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp);
   ...
   */
  function set(IStore _store, uint256 id, ClaimExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp);
  ...
  function decodeDynamic(
    ...
    bytes memory _blob
  ) internal pure returns (string memory player, string memory gameId) {
    uint256 _start;
    uint256 _end;
    unchecked {
    ...
    gameId = (string(SliceLib.getSubslice(_blob, _start, _end).toBytes()));
  }
  ...
  ) internal pure returns (ClaimExecutedData memory _table) {
    (_table.timestamp) = decodeStatic(_staticData);

    (_table.player, _table.gameId) = decodeDynamic(_encodedLengths, _dynamicData);
  }
   ...
   */
  function encodeDynamic(string memory player, string memory gameId) internal pure returns (bytes memory) {
    return abi.encodePacked(bytes((player)), bytes((gameId)));
  }
