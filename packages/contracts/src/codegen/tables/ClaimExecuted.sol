...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("ClaimExecuted")));
bytes32 constant ClaimExecutedTableId = _tableId;
...
library ClaimExecuted {
  ...
  function getMetadata() internal pure returns (string memory, string[] memory) {
    ...
    return ("ClaimExecuted", _fieldNames);
  }
  ...
  function emitEphemeral(uint256 id, ClaimExecutedData memory _table) internal {
    emitEphemeral(id, _table.timestamp, _table.player);
  }

  /** Emit the ephemeral event using the data struct (using the specified store) */
  function emitEphemeral(IStore _store, uint256 id, ClaimExecutedData memory _table) internal {
