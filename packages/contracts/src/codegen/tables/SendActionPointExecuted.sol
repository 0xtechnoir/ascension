...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("SendActionPointE")))
);
ResourceId constant SendActionPointExecutedTableId = _tableId;
...
library SendActionPointExecuted {
   ...
   */
  function set(uint256 id, uint256 timestamp, uint32 gameId, string memory sender, string memory reciever) internal {
    bytes memory _staticData = encodeStatic(timestamp, gameId);

    PackedCounter _encodedLengths = encodeLengths(sender, reciever);
    bytes memory _dynamicData = encodeDynamic(sender, reciever);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(uint256 id, uint256 timestamp, uint32 gameId, string memory sender, string memory reciever) internal {
    bytes memory _staticData = encodeStatic(timestamp, gameId);
   ...
   */
  function set(uint256 id, SendActionPointExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);

    PackedCounter _encodedLengths = encodeLengths(_table.sender, _table.reciever);
    bytes memory _dynamicData = encodeDynamic(_table.sender, _table.reciever);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, SendActionPointExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
   ...
   */
  function set(IStore _store, uint256 id, SendActionPointExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
  ...
  function decodeDynamic(
    ...
    bytes memory _blob
  ) internal pure returns (string memory sender, string memory reciever) {
    uint256 _start;
    uint256 _end;
    unchecked {
      _end = _encodedLengths.atIndex(0);
    }
    sender = (string(SliceLib.getSubslice(_blob, _start, _end).toBytes()));

    _start = _end;
    unchecked {
      _end += _encodedLengths.atIndex(1);
    }
    reciever = (string(SliceLib.getSubslice(_blob, _start, _end).toBytes()));
  }
  ...
  ) internal pure returns (SendActionPointExecutedData memory _table) {
    (_table.timestamp, _table.gameId) = decodeStatic(_staticData);

    (_table.sender, _table.reciever) = decodeDynamic(_encodedLengths, _dynamicData);
  }
   ...
   */
  function encodeDynamic(string memory sender, string memory reciever) internal pure returns (bytes memory) {
    return abi.encodePacked(bytes((sender)), bytes((reciever)));
  }
