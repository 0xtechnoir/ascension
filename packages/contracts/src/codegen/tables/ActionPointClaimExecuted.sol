...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("ActionPointClaim")))
);
ResourceId constant ActionPointClaimExecutedTableId = _tableId;
...
library ActionPointClaimExecuted {
   ...
   */
  function setTimestamp(IStore _store, uint32 id, uint256 timestamp) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(id));
   ...
   */
  function set(uint32 id, ActionPointClaimExecutedData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint32 id, ActionPointClaimExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
   ...
   */
  function set(IStore _store, uint32 id, ActionPointClaimExecutedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
  ...
  ) internal pure returns (ActionPointClaimExecutedData memory _table) {
