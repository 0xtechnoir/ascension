...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("PlayerLeftGame")))
);
ResourceId constant PlayerLeftGameTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x0024020120040000000000000000000000000000000000000000000000000000
);

struct PlayerLeftGameData {
  uint256 timestamp;
  uint32 gameId;
  string player;
}

library PlayerLeftGame {
  /**
   ...
   */
  function set(uint256 id, PlayerLeftGameData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, PlayerLeftGameData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
   ...
   */
  function set(IStore _store, uint256 id, PlayerLeftGameData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
  ...
  ) internal pure returns (PlayerLeftGameData memory _table) {
