...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("PlayerDied")))
);
ResourceId constant PlayerDiedTableId = _tableId;
...
library PlayerDied {
   ...
   */
  function set(uint256 id, PlayerDiedData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, PlayerDiedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
   ...
   */
  function set(IStore _store, uint256 id, PlayerDiedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.gameId);
  ...
  ) internal pure returns (PlayerDiedData memory _table) {
