...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("PlayerSpawned")))
);
ResourceId constant PlayerSpawnedTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x002c040120040404000000000000000000000000000000000000000000000000
);
...
library PlayerSpawned {
   ...
   */
  function set(uint256 id, uint256 timestamp, uint32 x, uint32 y, uint32 gameId, string memory player) internal {
   ...
   * @notice Set the full data using individual values.
   */
  function _set(uint256 id, uint256 timestamp, uint32 x, uint32 y, uint32 gameId, string memory player) internal {
    bytes memory _staticData = encodeStatic(timestamp, x, y, gameId);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, PlayerSpawnedData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.timestamp, _table.x, _table.y, _table.gameId);
  ...
  function decodeStatic(
    bytes memory _blob
  ) internal pure returns (uint256 timestamp, uint32 x, uint32 y, uint32 gameId) {
    timestamp = (uint256(Bytes.slice32(_blob, 0)));

    x = (uint32(Bytes.slice4(_blob, 32)));

    y = (uint32(Bytes.slice4(_blob, 36)));

    gameId = (uint32(Bytes.slice4(_blob, 40)));
  }
  ...
  ) internal pure returns (PlayerSpawnedData memory _table) {
    (_table.timestamp, _table.x, _table.y, _table.gameId) = decodeStatic(_staticData);
   ...
   */
  function encodeStatic(uint256 timestamp, uint32 x, uint32 y, uint32 gameId) internal pure returns (bytes memory) {
    return abi.encodePacked(timestamp, x, y, gameId);
  }
