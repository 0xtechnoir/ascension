...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("ActivityLog")));
bytes32 constant ActivityLogTableId = _tableId;
...
library ActivityLog {
  ...
  function getMetadata() internal pure returns (string memory, string[] memory) {
    ...
    _fieldNames[0] = "message";
    return ("ActivityLog", _fieldNames);
  }
  ...
  /** Get message */
  function get(bytes32 key) internal view returns (string memory message) {
  ...
  function get(IStore _store, bytes32 key) internal view returns (string memory message) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
  ...
  /** Set message */
  function set(bytes32 key, string memory message) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;

    StoreSwitch.setField(_tableId, _keyTuple, 0, bytes((message)));
  }

  /** Set message (using the specified store) */
  function set(IStore _store, bytes32 key, string memory message) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;

    _store.setField(_tableId, _keyTuple, 0, bytes((message)));
  }
  ...
  function encode(string memory message) internal pure returns (bytes memory) {
    uint40[] memory _counters = new uint40[](1);
    _counters[0] = uint40(bytes(message).length);
    PackedCounter _encodedLengths = PackedCounterLib.pack(_counters);

    return abi.encodePacked(_encodedLengths.unwrap(), bytes((message)));
  }
